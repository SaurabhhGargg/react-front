...
class MyAccount extends Component {
  ...
  componentDidMount() {
    ...
    var comment = JSON.parse(localStorage.getItem("login"));
    ...
    if (comment) 
    {
      ...
      console.log("comment retrieve : ", comment.token.email);
      EcommService.getAccountDetails(comment.token.email).then((res) => {
        console.log(res);
        let user = res.data.message[0];
        this.setState({
  ...
  validate = () => {
    ...
    if (!this.state.mobileNumber) {
      MobileNumberError = "Mobile Number cannont be empty";

    }
    if (!this.state.address1) {
      AddressError = " Address cannont be blank";
    }
    if (!this.state.city) {
      CityError = " City cannont be blank";
    }
    if (!this.state.states) {
      StateError = " Select the State";
    }
    if (!this.state.zip) {
      ZipError = "Zip cannont be blank";
    }
    if (
      NameError ||
      EmailError ||
  ...
  stopSubmission = (e) => {
    ...
    if (isValid) {
      var comment = JSON.parse(localStorage.getItem("login"));
    const token = localStorage.getItem('login');
    console.log(comment.token.email);
      axios
        .put(`${'http://localhost:8081'}/user/${comment.token.email}`, data)
        .then(res => {
          this.setState({
      ...
      <div>
        ...
        <div className="container mt-2">
          ...
          <form method="POST" onSubmit={this.stopSubmission}> 
            ...
            <div className="text-danger" title="MobileNumberError">{this.state.MobileNumberError}</div>
            <div>
              <label> Address</label>
              <input
                name="Address"
            ...
            <div className="form-row">
              ...
              <div>
                 ...
                 <div>
                    <label>City</label>
                    <select class="form-control" placeholder={this.state.city} value={this.state.city} onChange={(a)=>{
                      this.setState({
                        city:a.target.value
                      })
                  }}>
              ...
              <div>
                ...
                <div className="text-danger" title="ZipError">{this.state.ZipError}</div>
              </div>
            </div>
            <br />
            ...
            <br />
            <div>
              <button type="submit" className="btn btn-primary" title="sign">
                Save
              </button>{"   "}

              <Link to="/">
                <button onClick={this.logout} className="btn btn-primary pull-right">
                  Logout
                </button>
              </Link>
            </div>
